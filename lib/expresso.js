...
exports.run = function(callerOptions){

    setupEnvironment();

    Object.keys(callerOptions).forEach(function(option){
        if(options[option] !== undefined){
            options[option] = callerOptions[option];
        }
    });
...
function reportCoverageTable(cov) {
    ...
    for (var name in cov) {
        if (name.match(file_matcher)) {
            var file = cov[name];
            if ((file.coverage < 100) || !options.quiet) {
               print('\n   [bold]{' + name + '}:');
               print(file.source);
               sys.print('\n');
            }
...
function reportCoverageJson(cov) {
    ...
    fs.writeFileSync(options.jsonFile, JSON.stringify(report), "utf8");
}
...
function runSuite(title, tests, fn) {
    // Keys
    var keys = options.only.length
        ? options.only.slice(0)
        : Object.keys(tests);
            ...
            if (test) {
                try {
                    ...
                    if (options.serial) {
                        ...
                        var args = options.testArgs.slice();
                        args.push(beforeExit);
...
function report() {
    ...
    if (failures) {
        ...
        if (options.serial) print('');
        print('\n   [green]{100%} ' + testcount + ' tests\n');
        notify('100% ok');
    }
    if (typeof _$jscoverage === 'object') {
        populateCoverage(_$jscoverage);
        if (!hasFullCoverage(_$jscoverage) || !options.quiet) {
            (options.jsonCoverage ? reportCoverageJson(_$jscoverage) : reportCoverageTable(_$jscoverage));
        }
    }
}
...
function monkeyPatchAssert(){
    ...
    assert.response = function(server, req, res, msg){
        ...
        if (!server.fd) {
            ...
            server.listen(server.__port = options.port++, '127.0.0.1', check);
        ...
        // Create client
        if (!server.fd) {
            server.listen(server.__port = options.port++, '127.0.0.1', issue);
        } else {
            issue();
        }
